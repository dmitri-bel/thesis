...
parser.add_argument("--model", default="transformer", type=str, help="description model")
parser.add_argument("--task", type=constants.Task, default=constants.Task.NEXT_ACTIVITY,  help="task name")
...
if __name__ == "__main__":
    ...
    log_path = f"{log_path}/training_{args.task.value}.csv"

    result_path = f"{args.result_dir}/{args.dataset}"
    if not os.path.exists(result_path): os.makedirs(result_path)
    result_path = f"{result_path}/results_{args.task.value}"

    model_path = f"{args.model_dir}/{args.dataset}"
    if not os.path.exists(model_path): os.makedirs(model_path)
    model_path = f"{model_path}/{args.task.value}"

    tsb_path = f"{args.tsb_dir}/{args.dataset}"
    if not os.path.exists(tsb_path): os.makedirs(tsb_path)
    tsb_path = f"{tsb_path}/{args.task.value}/" + dt.datetime.now().strftime("%Y%m%d-%H%M%S")
