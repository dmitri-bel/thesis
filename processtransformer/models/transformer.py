...
def get_outcome_transformer_model(max_case_length, vocab_size, output_dim, 
    embed_dim = 36, num_heads = 4, ff_dim = 64):
    inputs = layers.Input(shape=(max_case_length,))
    x = TokenAndPositionEmbedding(max_case_length, vocab_size, embed_dim)(inputs)
    x = TransformerBlock(embed_dim, num_heads, ff_dim)(x)
    ...
    outputs = layers.Dense(output_dim, activation="linear")(x)
    transformer = tf.keras.Model(inputs=inputs, outputs=outputs,
        name = "outcome_transformer")
    return transformer

def get_outcome_lstm_model(max_case_length, vocab_size, output_dim, 
    embed_dim = 36, ff_dim = 64):
    inputs = layers.Input(shape=(max_case_length,))
    x = TokenAndPositionEmbedding(max_case_length, vocab_size, embed_dim)(inputs)
    x = layers.LSTM(ff_dim)(x)
    x = layers.Dropout(0.1)(x)
    x = layers.Dense(64, activation="relu")(x)
    x = layers.Dropout(0.1)(x)
    outputs = layers.Dense(output_dim, activation="linear")(x)
...
def get_outcome_cnn_model(max_case_length, output_dim):
    inp = layers.Input(shape=(max_case_length,))
    x = layers.BatchNormalization()(inp)
    x = layers.Dropout(0.1)(x)
    x = tfa.layers.WeightNormalization(layers.Dense(4096))(x)
    x = layers.Reshape((256, 16))(x)
    x = layers.BatchNormalization()(x)
    x = layers.Dropout(0.1)(x)
    x = tfa.layers.WeightNormalization(layers.Conv1D(filters = 16, kernel_size = 5, use_bias = False, padding = 'SAME'))(x)
    x = layers.AveragePooling1D(pool_size = 2)(x)
    xs = x
    x = layers.BatchNormalization()(x)
    x = layers.Dropout(0.1)(x)
    x = tfa.layers.WeightNormalization(layers.Conv1D(filters = 16, kernel_size = 3, use_bias = True, padding = 'SAME'))(x)
    x = layers.BatchNormalization()(x)
    x = layers.Dropout(0.1)(x)
    x = tfa.layers.WeightNormalization(layers.Conv1D(filters = 16, kernel_size = 3, use_bias = True, padding = 'SAME'))(x)
    x = layers.Multiply()([x, xs])
    x = layers.MaxPool1D(pool_size = 4, strides = 2)(x)
    x = layers.Flatten()(x)
    x = layers.BatchNormalization()(x)
